...
function applyPrefs(p) {
  ...
  // FBF scheduling + trail/step watchdogs
  if (Number.isFinite(p.fbfDelayFrames)) window.FBF_DELAY_FRAMES = Number(p.fbfDelayFrames);
  if (Number.isFinite(p.fbfLenFrames))   window.FBF_LEN_FRAMES   = Number(p.fbfLenFrames);
  if (Number.isFinite(p.ballMaxStep))    window.BALL_MAX_STEP    = Number(p.ballMaxStep);
  if (Number.isFinite(p.trailIdleMs))    window.TRAIL_IDLE_MS    = Number(p.trailIdleMs);
...
export function mountPrefs(hostEl) {
  ...
  // restore saved playback mode and reflect it globally early
  try {
    const savedMode = localStorage.getItem('shot_play_mode');
    if (savedMode) prefs.playMode = savedMode;
  } catch {}
  window.SHOT_PLAY_MODE = prefs.playMode || window.SHOT_PLAY_MODE || 'Slow';

  // build UI (do this BEFORE wiring any listeners)
  panel.innerHTML = `<h3>Preferences</h3>`;

  // ===== Playback & source =====
  panel.appendChild(sectionTitle('Playback & source'));

  // playback mode
  panel.appendChild(
    makeRadioGroup(
      'pf_play', 'Playback mode',
      [['Slow','Slow-mo'], ['fbf','Frame-by-frame']],
      (prefs.playMode ?? String(window.SHOT_PLAY_MODE || 'Slow')),
      'How to play after release.'
    )
  );
  ...
  // source buttons
  {
    ...
    row.innerHTML = `
      <div style="display:flex; gap:6px; flex-wrap:wrap;">
        <button id="pf_src_upload" type="button" style="flex:1;">Upload…</button>
        <button id="pf_src_camera" type="button" style="flex:1;">Use camera</button>
        <button id="pf_src_stop"   type="button" style="flex:1;">Stop camera</button>
      </div>
      <div id="pf_src_hint" style="margin-top:6px; opacity:.8;"></div>
    `;
    panel.appendChild(row);

    // safe wiring once DOM nodes exist
    queueMicrotask(() => {
      const get = sel => panel.querySelector(sel);

      const hint     = get('#pf_src_hint');
      const uploadBt = get('#pf_src_upload');
      const camBt    = get('#pf_src_camera');
      const stopBt   = get('#pf_src_stop');
      const allowCam = get('#pf_allow_cam'); // may get added later; guarded reads

      const canCamera = !!(navigator.mediaDevices?.getUserMedia);
      const httpsOk   = (location.protocol === 'https:' || location.hostname === 'localhost');

      if (uploadBt) uploadBt.addEventListener('click', () => {
        const input = document.getElementById('videoInput');
        if (!input) { alert('Upload input not found'); return; }
        input.click();
      });

      if (camBt) camBt.addEventListener('click', async () => {
        const allow = allowCam ? allowCam.checked : !!prefs.allowCamera;
        if (!allow) { if (hint) hint.textContent = 'Enable “Allow camera access” first.'; return; }
        if (!canCamera) { if (hint) hint.textContent = 'Camera not supported.'; return; }
        if (!httpsOk)   { if (hint) hint.textContent = 'Camera requires HTTPS (or localhost).'; return; }
        if (hint) hint.textContent = 'Requesting camera…';
        try { await window.useCamera?.(); if (hint) hint.textContent = 'Camera active.'; }
        catch (e) { if (hint) hint.textContent = 'Camera failed: ' + (e?.message || e); }
      });

      if (stopBt) stopBt.addEventListener('click', () => {
        try { window.stopCamera?.(); if (hint) hint.textContent = 'Camera stopped.'; }
        catch (e) { if (hint) hint.textContent = 'Stop failed: ' + (e?.message || e); }
      });

      function updateCamBtns() {
        const allow = allowCam ? allowCam.checked : !!prefs.allowCamera;
        const on = allow && canCamera && httpsOk;
        if (camBt)  camBt.disabled  = !on;
        if (stopBt) stopBt.disabled = !on;
      }
      updateCamBtns();

      panel.addEventListener('change', (e) => {
        if (e.target && e.target.id === 'pf_allow_cam') updateCamBtns();
      });
    });
  }
  ...
  // ===== FBF window / Trail guards =====
  panel.appendChild(sectionTitle('Shot window & guards'));
  panel.appendChild(makeNumber('pf_fbfDelay', 'FBF delay (frames)', 0, 60, 1, (prefs.fbfDelayFrames ?? Number(window.FBF_DELAY_FRAMES ?? 12)),
    'Frames after release to start FBF.'));
  panel.appendChild(makeNumber('pf_fbfLen', 'FBF length (frames)', 8, 90, 1, (prefs.fbfLenFrames ?? Number(window.FBF_LEN_FRAMES ?? 28)),
    'Frames to capture for the arc window.'));
  panel.appendChild(makeNumber('pf_ballMaxStep', 'Max ball step (px/frame)', 10, 120, 1, (prefs.ballMaxStep ?? Number(window.BALL_MAX_STEP ?? 40)),
    'Clamp/interpolate jumps larger than this.'));
  panel.appendChild(makeNumber('pf_idleMs', 'Trail idle finalize (ms)', 300, 2000, 50, (prefs.trailIdleMs ?? Number(window.TRAIL_IDLE_MS ?? 900)),
    'Finalize shot when trail is idle after prox exit.'));
  ...
  panel.addEventListener('input', (e) => {
    if (e.target.matches('input[type="range"]')) syncOutput(e.target.id);
  });
  ...
  function readAndApply() {
    ...
    // FBF window + trail/step watchdog
    p.fbfDelayFrames = Number($('pf_fbfDelay').value);
    p.fbfLenFrames   = Number($('pf_fbfLen').value);
    p.ballMaxStep    = Number($('pf_ballMaxStep').value);
    p.trailIdleMs    = Number($('pf_idleMs').value);
    ...
    // playback mode (apply + persist once)
    const playEl = panel.querySelector('input[name="pf_play"]:checked');
    p.playMode = playEl ? playEl.value : 'Slow';
    window.SHOT_PLAY_MODE = p.playMode;
    try { localStorage.setItem('shot_play_mode', p.playMode); } catch {}
  ...
  const closeBtn = panel.querySelector('#pf_close');
  if (closeBtn) closeBtn.addEventListener('click', () => panel.style.display='none');

  const resetBtn = panel.querySelector('#pf_reset');
  ...
  // first apply + sync range outputs
  applyPrefs(prefs);
